...
.controller('rootCtrl', ['$scope', '$rootScope', '$uibModal', 'User', '$timeout', '$http', 'ngProgressFactory', '$window', '$localStorage',  function($scope, $rootScope, $uibModal, User, $timeout, $http, ngProgressFactory, $window, $localStorage) {
    ...
    $rootScope.apiURL = window.location.protocol + "//api." + window.location.host + "/";
    ...
    $scope.init = function () {
        ...
        $rootScope.menus = [];
        $http.get($rootScope.apiURL + 'category/getlistcategories')
            .success(function(data, status, headers, config) {
        ...
        $http.get($rootScope.apiURL + 'user/getlistusers/0/11')
            .success(function(data, status, headers, config) {
...
.controller('ComposeCtrl', ['$scope','$http','$location','$rootScope', '$uibModal', '$uibModalInstance', '$state', function($scope,$http,$location,$rootScope, $uibModal, $uibModalInstance, $state) {
    ...
    $scope.save = function(){
        ...
        $http.post($rootScope.apiURL + 'post/createpost', params )
            .success(function(data, status, headers, config) {
                if(data.error == 0){
...
.controller('HomeCtrl', ['$scope','$http','$location', 'Post', '$timeout', '$rootScope', 'ngProgressFactory', function($scope,$http,$location, Post, $timeout, $rootScope, ngProgressFactory) {
    ...
    $scope.nextPage = function() {
        ...
        $http.get($rootScope.apiURL + 'post/getlistposts/'+$scope.offset+'/'+$scope.numPerPage)
            .success(function(data, status, headers, config) {
                if(data.error == 0){
                    var items = data.data;
...
.controller('CategoryCtrl', ['$scope','$http','$location', 'Post', '$timeout', '$rootScope', '$stateParams', 'ngProgressFactory',
    ...
    $scope.nextPage = function() {
        if ($scope.busy) return;
        $scope.busy = true;
        $http.get($rootScope.apiURL + 'post/getpostsbycategoryslug/'+slug+'/'+$scope.offset+'/'+$scope.numPerPage)
            .success(function(data, status, headers, config) {
                var items = data.data;
                for (var i = 0; i < items.length; i++) {
                     $scope.posts.push(items[i]);
    ...
    $http.get($rootScope.apiURL + 'post/gethotposts/0/4')
	.success(function(data, status, headers, config) {
